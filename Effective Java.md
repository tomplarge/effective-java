...
- use unbounded wildcard types instead of raw type when element type doesn't magtter (e.g. use `Set<?>` instead of `Set` )
```java
...
p.start().setYear(1); // modifies internals of p too
```
...
- Must also return defensive copies of internal fields:
```java
public Date start() {
...
- 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTE0ODA4ODkxMCwxOTU5OTExNTYzLC0yOD
